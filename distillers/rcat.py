...
@register_distiller
class RCAT(BaseDistiller):
    requires_feat = False
    def __init__(self, student, teacher, criterion, args, **kwargs):
        super(RCAT, self).__init__(student, teacher, criterion, args)
        self.args = args
        ...
        self.cat_loss_weight = args.cat_loss_weight
    ...
    def forward(self, image, target, epoch=1, **kwargs):
        ...
        with torch.no_grad():
            ...
            if self.teacher_type == 'dinov2s_finetuehead':
                ...
                feature_teacher = teacher_tokens.reshape(teacher_tokens.size(0), int(teacher_tokens.size(1)**0.5), int(teacher_tokens.size(1)**0.5), teacher_tokens.size(2)).permute(0, 3, 1, 2)  # [B, D, H, W]
                feat_tea = feature_teacher 
            elif self.teacher_type == 'mixer_b16_224':
                teacher_feat = self.teacher.forward_features(image) 
                teacher_tokens = teacher_feat                
                feature_teacher = teacher_tokens.reshape(teacher_tokens.size(0), int(teacher_tokens.size(1)**0.5), int(teacher_tokens.size(1)**0.5), teacher_tokens.size(2)).permute(0, 3, 1, 2)  # [B, D, H, W]
                feat_tea = feature_teacher 
        ...
        feat_stu = feature_student['hca_feat']
        cam_tea = get_CAM(model=self.teacher,feat_map=feat_tea)
        # cam_tea = get_CAM(model=self.teacher,feat_map=teacher_tokens,arch = self.teacher_type)
        cam_stu = get_CAM(model=self.student,feat_map=feat_stu)
        loss_cat = CAT_loss(cam_stu,cam_tea,CAM_RESOLUTION=4)
        losses_dict["loss_cat"] = self.cat_loss_weight * loss_cat
        ...
        if teacher_tokens is not None:
            ...
            losses_dict["loss_feat"] = self.feat_loss_weight * loss_feat
        ...
        #     losses_dict["loss_kd"] = self.logits_loss_weight * loss_logits
