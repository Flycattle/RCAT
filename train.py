...
from datasets import get_cub200_dataloader
from datasets import get_tinyimagenet_dataloader
...
parser.add_argument('--feat-loss-weight', default=50., type=float)
parser.add_argument('--cat-loss-weight', default=300., type=float)
...
parser.add_argument('--dkd-temperature', default=4, type=float) #原设置为1
...
parser.add_argument('--dataset', '-d', metavar='NAME', default='cub200',
...
parser.add_argument('--decay-milestones', type=list[int], default=[100,160,220,280],
                    help='MultiStepLR decay milestones (comma-separated epochs)')
...
def main():
    ...
    log_path =args.distiller + args.dataset  + args.teacher + '_' + args.model + '_' +  'train.log' 
    ...
    teacher = None
    if args.teacher:
        if args.teacher == 'dinov2b':
            teacher = torch.hub.load('/nfs4/wangyb/projects/dinokd/dinov2', 'dinov2_vitb14', source='local', force_reload=False)
        elif args.teacher == 'dinov2s':
            teacher = torch.hub.load('/nfs4/wangyb/projects/dinokd/dinov2', 'dinov2_vits14_lc', source='local', layers=1, force_reload=False)
            teacher = torch.hub.load('/nfs4/wangyb/projects/dinokd/dinov2', 'dinov2_vits14', source='local', force_reload=False)
        elif args.teacher == 'dinov2s_finetuehead':
            # teacher = torch.hub.load('/nfs4/wangyb/projects/dinokd/dinov2', 'dinov2_vits14_lc', source='local', layers=1, force_reload=False)
            
            # 加载ofa需要带logits的dinov2_vits14模型，
            backbone = torch.hub.load('/nfs4/wangyb/projects/dinokd/dinov2', 'dinov2_vits14', source='local', force_reload=False)            
            feat_dim = 384  # 或者 backbone.num_features
            # 创建新的 head 层
            num_classes = args.num_classes  # CIFAR-100 有 100 个类别
            head = nn.Linear(feat_dim, num_classes)
            # 加载预训练的 head 权重
            head_checkpoint_path = args.teacher_pretrained
            head_state_dict = torch.load(head_checkpoint_path, map_location='cpu')
            head.load_state_dict(head_state_dict)
            # 构建完整的 teacher 模型
            teacher = Dinov2Teacher(backbone, head) 

        elif args.teacher == 'dinov3s':
        #     dinov3_vits16 = torch.hub.load(REPO_DIR, 'dinov3_vits16', source='local', weights=<CHECKPOINT/URL/OR/PATH>)
            teacher = torch.hub.load('/nfs4/wangyb/projects/dinokd/dinov3', 'dinov3_vits16', source='local', weights='/nfs4/wangyb/projects/dinokd/dinov3/dinov3/checkpoint/dinov3_vits16_pretrain_lvd1689m-08c60483.pth')
        elif args.teacher == 'swin-tiny-finetuned-cifar100':
            # Load model directly
            import transformers
            from transformers import AutoImageProcessor, AutoModelForImageClassification

            processor = AutoImageProcessor.from_pretrained("MazenAmria/swin-tiny-finetuned-cifar100")
            teacher = AutoModelForImageClassification.from_pretrained("MazenAmria/swin-tiny-finetuned-cifar100")
        else: 
            ...
            if Distiller.requires_feat:
                ...
                print("Registered models:", list_models(pretrained=False))
    ...
    if args.dataset == 'cifar100':
        ...
        loader_train,loader_eval,_ = get_cub200_dataloader(args.batch_size,args.batch_size,args.workers)
    elif args.dataset == 'tiny-imagenet-200':
        loader_train,loader_eval,_ = get_tinyimagenet_dataloader(args.batch_size,args.batch_size,args.workers)
    ...
    distiller = Distiller(model, teacher=teacher, criterion=train_loss_fn, args=args, num_data=len(loader_train)if loader_train is not None else len(dataset_train) )
    ...
    if args.dataset != 'cub200' and args.dataset != 'tiny-imagenet-200':
    ...
    if args.rank == 0:
        os.system(f'mv {log_path} {output_dir}')
...
def train_one_epoch(
    ...
    for batch_idx, (input, target, *additional_input) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            # output, losses_dict = distiller(input, target, epoch=epoch,*additional_input)
            # output, losses_dict = distiller(input, target, epoch=epoch,)
